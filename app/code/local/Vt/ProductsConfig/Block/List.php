...
class Vt_ProductsConfig_Block_List extends Mage_Catalog_Block_Product_List
{
    ...
    public function getFlycartAssociatedProduct(){
      ...
      /*if (!$this->_productlist){       
		...
		$productFly = Mage::getSingleton('core/session')->getProductFly(); 
		$productFly = unserialize($productFly);
		//print_r($productFly);
		if ($productFly){       
             ...
             for( $i=0; $i< count($productFly); $i++){  
				
                $product = Mage::getModel('catalog/product')->load($productFly[$i]); 
                $this->_productlist[$product->getId()] = $helper->getFlycartProductData($product);
	...
	public function getProductIdFlycart(){		
		$storage_arr = array();
		$storage = array();
		$storage_arr = Mage::getSingleton('core/session')->getRbanhShipping(); 
		$storage_arr = unserialize($storage_arr);
		foreach ($this->_productCollection as $_product){   
			//$tmp = array();
			//$tmp[0] = $_product->getId();
			/*if (!in_array($tmp[0], $storage_arr)) {
				$storage = array_merge($tmp, $storage_arr);				
			}*/	
			//$storage = array_merge($tmp, $storage_arr);
		...
		/*for($i =0; $i< count($product_ids); $i++) {
			$tmp = array();
			$tmp[$i] = $product_ids[$i];
			if (!array_key_exists($product_ids[$i], $storage_arr)) {
				$storage = array_merge($tmp, $storage_arr);
			}
		}*/
	...
	protected function _getProductCollection()
	{
		if (is_null($this->_productCollection)) {
			...
			if($this->filter == 'deals' || $this->filter == 'sale'){
				if($this->getIsenabledTimeDeal()==1){		
					$datestsp = $this->getLimitationTimeStart();	
						$collection->addAttributeToFilter('special_price', array('neq' => ""));	
						$datestsp = date('Y-m-d', $datestsp);	
						...
						$datefnsp = date('Y-m-d', $datefnsp);
						$collection->addAttributeToFilter('special_to_date',array('or'=> array(0 => array('date' => true, 'from' => $datefnsp),1 => array('isNot' => new Zend_Db_Expr('null')))), 'left');
					}else{					 
					$todayDate  = Mage::app()->getLocale()->date()->toString(Varien_Date::DATETIME_INTERNAL_FORMAT);
					$tomorrow = mktime(0, 0, 0, date('m'), date('d')+1, date('y'));
					$dateTomorrow = date('m/d/y', $tomorrow);
	...
	private function _addCategoryFilter(& $collection, $category_ids){
		...
		$storage_arr = array();		
		$storage_arr = Mage::getSingleton('core/session')->getProductFly(); 
		$storage_arr = unserialize($storage_arr);
		...
		for($i=0; $i< count($product_ids); $i++) {
			...
			if (!in_array($product_ids[$i], $storage_arr)) {
				$storage_arr = array_merge($tmp, $storage_arr);
			}
		}
		
		Mage::getSingleton('core/session')->setProductFly(serialize($storage_arr));
		//$productFly = Mage::getSingleton('core/session')->getProductFly(); 
		//$productFly = unserialize($productFly);
